...
function extractKeywords(text, topN = 20) {
  ...
  uniFreq.forEach((count, term) => {
    // skip unigrams that are fully contained in an already-added phrase-worthy term
    candidates.push({ term, count, isPhrase: false });
  });

  candidates.sort((a, b) => b.count - a.count || b.term.length - a.term.length);

  // de-dupe: drop unigram if it's a substring of a kept phrase
  const kept = [];
...
// Indicative-only estimate. Combines keyword match with basic resume-structure
// signals. This is NOT a prediction of real hiring outcomes — recruiter behavior,
// company-specific ATS configs, and market conditions all matter far more than
// what any keyword tool can see. Score is intentionally capped away from 0/100
// so it never reads as a guarantee.
...
// Suggests a job-search term from the resume alone (no JD needed). Prefers
// role-sounding phrases (containing words like "developer", "engineer", etc.)
// over generic skill keywords, so the search is closer to an actual job title.
...
function Scanner({ session, profile, onScanUsed, onUpgraded, onOpenAdmin, onLogout }) {
  ...
  // Auto-suggest a job search term from the resume alone, so users can find
  // matching roles without needing to run a full JD scan first. Only overrides
  // the field while the user hasn't typed their own search term.
  useEffect(() => {
    ...
    <div className="ats-page-pad" style={styles.page}>
      ...
      <div className="ats-root" style={styles.container}>
        <div style={styles.accountBar}>
          <span className="ats-mono" style={styles.accountEmail}>{session.user.email}</span>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {session.user.email === ADMIN_EMAIL && (
              <button style={styles.accountBtn} onClick={onOpenAdmin}>
                <ShieldCheck size={13} /> Admin
              </button>
            )}
            <button style={styles.accountBtn} onClick={onLogout}>
...
const styles = {
  ...
  accountEmail: { color: "#7C8896", fontSize: 12 },
  accountBtn: {
    display: "flex",
    alignItems: "center",
    gap: 5,
...
// ---------- Auth wrapper ----------
// Handles login gate, profile creation/sync with the database, and switching
// between the scanner and the admin-only view. This is the actual default export.
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      const { data: created, error: insertError } = await supabase
        .from("profiles")
        .insert({ id: session.user.id, email: session.user.email, is_pro: false, scan_count: 0 })
        .select()
        .single();
  ...
  const handleScanUsed = async () => {
    if (!session || !profile) return;
    const next = profile.scan_count + 1;
    setProfile({ ...profile, scan_count: next });
    await supabase.from("profiles").update({ scan_count: next }).eq("id", session.user.id);
  };
  ...
  if (authLoading) {
    return <div style={{ minHeight: "100vh", background: "#10151C" }} />;
  }
  ...
  if (view === "admin") {
    return <Admin session={session} onBack={() => setView("scanner")} />;
  }
